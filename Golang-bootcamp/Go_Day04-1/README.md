...
several vending machines, 5 delicious kinds of candy:
...
In source code abbreviations are used, so it's CE, AA, NT and so on"

<h3 id="ex00">Exercise 00: </h3>

specification for a protocol between vending machine and a server:
...
In order to recreate the server, you have to use this spec to produce a bunch of Go code which will actually implement the backend part.
...
<h3 id="ex01">Exercise 01:</h3>

you need to implement a certificate authentication for the server as well as a test client which will be able to query your API using a self-signed certificate and a local security authority to "verify" it on both sides.
